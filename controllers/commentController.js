    ...
    if(client_id == -1) res.status(500).send({message: 'You are admin'})
        ...
        if(client_id) {
            ...
            while(x < badWord.length) {
                if(comment_content.search(badWord[x]) != -1) {
                    res.status(500).send({message: 'You have typed forbidden word'})
                    return
                }
                x += 1
            }
            var change = await pool.query('UPDATE song SET num_of_comments = num_of_comments + 1, last_updated_stamp = current_timestamp WHERE song_id = $1', [song_id])
            var comment = await pool.query('INSERT INTO comment(comment_id, client_id, song_id, comment_content, last_updated_stamp, created_stamp) VALUES (default, $1, $2, $3, current_timestamp, default)', [client_id, song_id, comment_content])
            if(comment.rowCount) {
...
export const post_deleteComment = async (req, res) => {
    ...
    if(client_id == -1 || song.rows[0].client_id == client_id) {
        ...
        if(comment) {
            if(change) res.status(201).send({message: 'Delete comment successful'})
            else res.status(500).send({message: 'Error in updating number of comment in song'})
        }
        else res.status(500).send({message: 'Error in adding comment'})
    }
...
export const post_editComment = async (req, res) => {
    ...
    if(client_id == -1 || client.rows[0].client_id == client_id) {
        var comment = await pool.query('UPDATE comment SET comment_content = $2, last_updated_stamp = current_timestamp WHERE comment_id = $1', [comment_id, comment_content])
        if(comment) res.status(201).send({message: 'Edit comment successful'})
...
export const getNumCommentinSong = async (req, res) => {
    const {song_id} = req.body
    var comment = await pool.query('SELECT num_of_comments FROM song WHERE song_id = $1', [song_id])
    if(comment) res.status(201).send(comment.rows[0].num_of_comments)
