...
export const post_addNewArtist = async (req, res) => {
    const {artist_name, artist_info} = req.body
    
    try {
        var artist = await pool.query('INSERT INTO artist(artist_id, artist_name, artist_info, birth_date, num_of_songs, last_updated_stamp, created_stamp) \
            VALUES(default, $1, $2, null, 0, null, default) RETURNING *', [artist_name, artist_info])
        console.log(req)
    } catch (err) {
        console.log(err.stack)
    }
